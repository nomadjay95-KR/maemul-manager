...
### 인앱 알림
- 앱 접속 시 메인 화면 상단에 다가오는 일정 배너 표시
- 오늘~7일 이내 캘린더 일정 자동 조회 (잔금·입주·계약서·중도금·기타 모든 종류)
- 오늘 일정 강조 (빨강 배경), 이번 주 일정 일반 표시
- 일정 클릭 시 캘린더 상세(수정) 페이지로 이동
- 다가오는 일정 없으면 배너 숨김

### 고객 외부 공유
- 매물 상세 → "고객 공유 링크 만들기" 버튼으로 공개 링크 생성
- 매물 목록 → "선택 공유" 모드로 여러 매물 묶음 공유
- 공개 페이지: 로그인 없이 링크만으로 매물 정보 열람
- 공개 필드만 표시 (비공개: 집주인 연락처/성향, 현관 비밀번호, 메모)
- 공개 페이지 하단에 중개사 연락처 고정 표시 (tel: 링크)
- 링크 관리: 활성/비활성 토글, 조회수 확인, 삭제
- 생성 시 카카오톡 공유 + 링크 복사
...
- 모바일/태블릿 최적화 반응형 디자인
- 탭 기반 대시보드: 메인 3탭 (매물장 / 문의장 / 캘린더) + 더보기 (통계 / 메모장 / 공유 링크 / 사용자 관리)
