  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
      ...
      <Field label="주소 *" error={errors.address?.message}>
        <Input {...register("address")} placeholder="예: 서울시 관악구 봉천동 123-4" />
      </Field>
      ...
      {dealType === "monthly" && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="월세 (만원) *" error={errors.monthly_rent?.message}>
            <Input type="number" {...register("monthly_rent")} placeholder="50" />
          </Field>
        </div>
      )}
      {dealType === "jeonse" && (
        <Field label="전세금 (만원) *" error={errors.jeonse_price?.message}>
          <Input type="number" {...register("jeonse_price")} placeholder="15000" />
        </Field>
      )}
      {dealType === "sale" && (
        <Field label="매매가 (만원) *" error={errors.sale_price?.message}>
          <Input type="number" {...register("sale_price")} placeholder="30000" />
        </Field>
      )}
      ...
      {propertyType === "villa" && (
        <div className="space-y-4">
          <h3 className="text-sm font-semibold border-b border-border pb-2">빌라 정보</h3>
          <div className="grid grid-cols-2 gap-3">
            <Field label="호수">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="연식 (년)">
              <Input type="number" {...register("building_age")} placeholder="15" />
            </Field>
            <Field label="대출 가능">
      ...
      {propertyType === "shop" && (
        <div className="space-y-4">
          <h3 className="text-sm font-semibold border-b border-border pb-2">상가 정보</h3>
          <div className="grid grid-cols-2 gap-3">
            <Field label="층수">
              <Input {...register("floor")} placeholder="1층" />
            </Field>
            <Field label="면적 (m²)">
              <Input type="number" step="0.01" {...register("area")} placeholder="45.5" />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="권리금 (만원)">
              <Input type="number" {...register("premium")} placeholder="5000" />
            </Field>
            <Field label="업종 제한">
              <Input {...register("business_restriction")} placeholder="음식점 불가" />
            </Field>
          </div>
        </div>
      )}
      ...
      {/* 연락처 / 보안 */}
      <div className="space-y-4">
        <h3 className="text-sm font-semibold border-b border-border pb-2">연락처 / 보안</h3>
        <Field label="집주인 연락처">
      ...
      {/* 기타 */}
      <div className="space-y-4">
        <h3 className="text-sm font-semibold border-b border-border pb-2">기타</h3>
        <Field label="특이사항">
          <Textarea {...register("notes")} placeholder="역세권 도보 5분" rows={2} />
        </Field>
        <Field label="메모">
