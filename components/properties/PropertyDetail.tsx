  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {p.images.length > 0 ? (
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
          {p.images.map((img) => (
            <div
              key={img.id}
              className="relative w-60 h-40 flex-shrink-0 rounded-lg overflow-hidden bg-muted"
            >
              <Image
                src={img.image_url}
                alt={p.address}
        ...
        <div className="flex items-center justify-center h-40 rounded-lg bg-muted text-muted-foreground text-sm">
          등록된 사진이 없습니다
        </div>
      )}
      ...
      <div>
        ...
        <h2 className="text-lg font-bold text-foreground">{p.address}</h2>
      </div>

      {/* 가격 */}
      <div className="rounded-lg bg-muted/50 p-4">
        <p className="text-sm text-muted-foreground mb-1">{DEAL_LABEL[p.deal_type]}</p>
        <p className="text-2xl font-bold text-foreground">{getPriceText(p)}</p>
        {p.deal_type === "monthly" && (
      ...
      {p.type === "villa" && (
        <section>
          <h3 className="text-sm font-semibold text-foreground mb-2">빌라 정보</h3>
          <div className="rounded-lg border border-border p-3">
      ...
      {p.type === "shop" && (
        <section>
          <h3 className="text-sm font-semibold text-foreground mb-2">상가 정보</h3>
          <div className="rounded-lg border border-border p-3">
      ...
      {(p.owner_phone || p.owner_personality || p.door_password) && (
        <section>
          <h3 className="text-sm font-semibold text-foreground mb-2">연락처 / 보안</h3>
          <div className="rounded-lg border border-border p-3">
      ...
      {(p.notes || p.memo) && (
        <section>
          <h3 className="text-sm font-semibold text-foreground mb-2">기타</h3>
          <div className="rounded-lg border border-border p-3">
